...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   fc_double_bracket_big_to_small.c                   :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: angelo <marvin@42lausanne.ch>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/30 12:35:15 by angelo            #+#    #+#             */
/*   Updated: 2022/09/30 20:14:17 by angelo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fc_double_bracket_big_to_small(t_info *info)
{
	...
	if (info->exe->pid_init == 0)
	{
		...
		if (info->exe->fd[0] < 0)
			fc_error_tmp(1, "Problem with open\n");
	if (write(info->exe->fd[0], buffer, ft_strlen(buffer)) < 0)
		fc_error_tmp(1, "Problem with write\n");
	if (close (info->exe->fd[0]) < 0)
		fc_error_tmp(1, "Problem with close(info->exe->fd[0])\n");
	if (close(info->exe->fd[1]) < 0)
